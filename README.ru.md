...
Теги хранятся как хэштеги (`#work`, `#shopping`) в поле notes напоминания и парсятся сервером:
...
> **Важно:** Это _не_ нативные теги Reminders. Apple не предоставляет управление тегами через EventKit API или AppleScript. Нативные теги (цветные метки в боковой панели Reminders) можно назначить только через интерфейс приложения. Теги, создаваемые этим сервером — это строки с хэштегами в поле notes. Они полностью работают для поиска и фильтрации через MCP-сервер, но не отображаются как нативные теги в приложении Reminders.

## Известные ограничения

- **Теги не нативные.** В EventKit нет API для нативных тегов Reminders. Теги хранятся как `#hashtag` в notes — полностью работают для поиска и фильтрации через MCP, но невидимы для системы тегов приложения Reminders. Подробнее в разделе [Теги](#теги).
